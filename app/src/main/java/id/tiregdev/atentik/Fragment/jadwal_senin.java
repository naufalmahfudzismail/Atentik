...
public class jadwal_senin extends Fragment
{
	...
	View v;
	String tokens;
	List<object_jadwal> jdwlsenin = new ArrayList<>();
	...
	public void setupAdapter()
	{
		...
		Call<List<object_jadwal>> call = client.jadwalMahasiswa("Bearer " + tokens, "Senin");
		call.enqueue(new Callback<List<object_jadwal>>()
		{
			...
			public void onResponse(Call<List<object_jadwal>> call, Response<List<object_jadwal>> response)
			{
				if (response.isSuccessful())
				{
					...
					{
						jdwlsenin.add(new object_jadwal(simpan.get(i).getNama_matkul(), simpan.get(i).getNama_dosen(), simpan.get(i).getJam_mulai() + " - " + simpan.get(i).getJam_selesai(), simpan.get(i).getRuangan(), "Senin"));
					}
					List<object_jadwal> rowListItem = jdwlsenin;
