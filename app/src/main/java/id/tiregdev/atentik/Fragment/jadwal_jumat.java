...
public class jadwal_jumat extends Fragment {

    RecyclerView rView;
    LinearLayoutManager lLayout;
    View v;
    String tokens;
    List<object_jadwal> jdwljumat = new ArrayList<>();
    ...
    public void setupAdapter(){
        ...
        Call<List<object_jadwal>> call = client.jadwalMahasiswa("Bearer " + tokens, "Jumat");
        call.enqueue(new Callback<List<object_jadwal>>() {
            ...
            public void onResponse(Call<List<object_jadwal>> call, Response<List<object_jadwal>> response) {
                if(response.isSuccessful())
                {
                    ...
                    {
                        jdwljumat.add(new object_jadwal(simpan.get(i).getNama_matkul(),simpan.get(i).getNama_dosen(),simpan.get(i).getJam_mulai() + " - " + simpan.get(i).getJam_selesai(), simpan.get(i).getRuangan(), "Jumat"));
                    }
                    List<object_jadwal> rowListItem = jdwljumat;
...
//        allItems.add(new object_jadwal(getString(R.string.mk1), "Kurain Jodi SHum., MI.","10.30 - 12.00", "3","AA301"));
