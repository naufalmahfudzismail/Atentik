...
public class jadwal_rabu extends Fragment {

    RecyclerView rView;
    LinearLayoutManager lLayout;
    View v;
    String tokens;
    List<object_jadwal> jdwlrabu = new ArrayList<>();
    ...
    public void setupAdapter(){
        ...
        Call<List<object_jadwal>> call = client.jadwalMahasiswa("Bearer " + tokens, "Rabu");
        call.enqueue(new Callback<List<object_jadwal>>() {
            ...
            public void onResponse(Call<List<object_jadwal>> call, Response<List<object_jadwal>> response) {
                if(response.isSuccessful())
                {
                    ...
                    {
                        jdwlrabu.add(new object_jadwal(simpan.get(i).getNama_matkul(),simpan.get(i).getNama_dosen(),simpan.get(i).getJam_mulai() + " - " + simpan.get(i).getJam_selesai(), simpan.get(i).getRuangan(), "Rabu"));
                    }
                    List<object_jadwal> rowListItem = jdwlrabu;
...
//        allItems.add(new object_jadwal(getString(R.string.mk5), "Kurain Jodi SHum., MI.","10.30 - 12.00", "3","AA301"));
