...
public class jadwal_kamis extends Fragment {

    RecyclerView rView;
    LinearLayoutManager lLayout;
    View v;
    String tokens;
    List<object_jadwal> jdwlkamis = new ArrayList<>();
    ...
    public void setupAdapter(){
        ...
        Call<List<object_jadwal>> call = client.jadwalMahasiswa("Bearer " + tokens, "Kamis");
        call.enqueue(new Callback<List<object_jadwal>>() {
            ...
            public void onResponse(Call<List<object_jadwal>> call, Response<List<object_jadwal>> response) {
                if(response.isSuccessful())
                {
                    ...
                    {
                        jdwlkamis.add(new object_jadwal(simpan.get(i).getNama_matkul(),simpan.get(i).getNama_dosen(),simpan.get(i).getJam_mulai() + " - " + simpan.get(i).getJam_selesai(),  simpan.get(i).getRuangan(), "Kamis"));
                    }
                    List<object_jadwal> rowListItem = jdwlkamis;
...
//        allItems.add(new object_jadwal(getString(R.string.mk2), "Dr. Geraldi Joni STr., MI.","08.00 - 10.30", "4","AA301"));
