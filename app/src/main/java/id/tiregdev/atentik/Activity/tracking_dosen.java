            ...
            public void onResponse(Call<List<object_lokasi>> call, Response<List<object_lokasi>> response) {
                if(response.isSuccessful())
                {
                    ...
                    for(int i = 0; i < simpan.size(); i++)
                    {
                        ...
                        lihatlokasi.add(new object_lokasi(simpan.get(i).getNama(),simpan.get(i).getNimornip(),"Sedang berada di " + simpan.get(i).getruangan() + " dari jam " + waktus, simpan.get(i).getPhoto()));
                    }
